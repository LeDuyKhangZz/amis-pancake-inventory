...
export function buildPlan({ products, inventory, variations, warehouseId }) {
  ...
  for (const row of inventory) {
    ...
    if (sku) inventoryBySku.set(sku, {
      total: row?.main_stock_quantity,
      orderable: row?.order_quantity
    });
  }
  ...
  for (const product of active) {
    ...
    const sourceQuantity = inventoryBySku.get(sku) || { total: 0, orderable: 0 };
    const totalQuantity = normalizeQuantity(sourceQuantity.total);
    const orderableQuantity = normalizeQuantity(sourceQuantity.orderable);
    if (!totalQuantity.ok) { skipped.push(reasonRow(product, `Số lượng tồn: ${totalQuantity.reason}`)); continue; }
    if (!orderableQuantity.ok) { skipped.push(reasonRow(product, `SL có thể đặt: ${orderableQuantity.reason}`)); continue; }
    ...
    if (targetMatches.length === 0) {
      create.push({
        ...
        remain_quantity: orderableQuantity.value,
        payload: makeProduct(product, totalQuantity.value, warehouseId)
      });
      continue;
    }
...
export async function commitSync(config, { fetchImpl = globalThis.fetch, createProducts = true } = {}) {
  const context = await loadContext(config, fetchImpl);
  const { plan, pancake } = context;
  const createQueue = createProducts ? plan.create.slice(0, config.createBatchSize) : [];
  const failures = [];
  let failed = 0;
  let created = 0;
  ...
  for (let index = 0; index < plan.update.length; index += INVENTORY_BATCH_SIZE) {
    ...
    try {
      await pancake.updateQuantities(batch.map((row) => ({
        variation_id: row.payload.variation_id,
        warehouse_id: row.payload.warehouse_id,
        remain_quantity: row.payload.total_quantity
      })), true);
      await pancake.updateQuantities(batch.map((row) => ({
        variation_id: row.payload.variation_id,
        warehouse_id: row.payload.warehouse_id,
        remain_quantity: row.payload.remain_quantity
      })), false);
  ...
  const remainingToCreate = createProducts ? Math.max(0, plan.create.length - created) : plan.create.length;
  console.info(JSON.stringify({
    event: "sync_commit",
    created,
