...
export function selectPancakeWarehouse(warehouses, configuredId = "") {
  ...
  const choices = warehouses.slice(0, 20).map((warehouse) => {
    const id = String(warehouse?.id ?? warehouse?.warehouse_id ?? "").trim() || "[không có ID]";
    const name = String(warehouse?.name ?? warehouse?.warehouse_name ?? "").trim() || "[không có tên]";
    return `${id} - ${name}`;
  }).join("; ");
  throw new PublicError(`Pancake có nhiều kho: ${choices}. Hãy cấu hình PANCAKE_WAREHOUSE_ID.`, {
    code: "PANCAKE_WAREHOUSE_AMBIGUOUS",
    status: 409
  });
}
...
export class PancakeClient {
  ...
  updateQuantities(rows, isActualRemainQuantity) {
    return requestJson(this.url("/variations/update_quantity"), {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json" },
      body: { is_actual_remain_quantity: isActualRemainQuantity, variations_warehouses: rows },
      fetchImpl: this.fetchImpl,
